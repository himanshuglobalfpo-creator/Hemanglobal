import { pgTable, text, integer, bigint, serial, boolean, doublePrecision, timestamp, jsonb } from "drizzle-orm/pg-core";
...
// are idempotent and resumable (re-running skips items already 'ok').
export const JOB_STATUSES = ["queued", "running", "completed", "failed"] as const;
export type JobStatus = (typeof JOB_STATUSES)[number];
export type JobItemResult = { id: number; status: "ok" | "error" | "skipped"; message?: string };
...
export const BATCH_KINDS = [
  "invoice.void", "invoice.send", "invoice.reminder_exempt",
  "bill.void", "bill.reminder_exempt",
  "customer.statement",
] as const;
...
export const createJobSchema = z.object({
  ...
  ids: z.array(z.number().int().positive()).min(1, "Select at least one item").max(1000),
...
export const invoices = pgTable("invoices", {
  ...
  recurringTemplateId: integer("recurring_template_id"), // set when auto-generated by a recurring template
  reminderExempt: boolean("reminder_exempt").notNull().default(false), // excluded from statement/reminder batches
  sentAt: text("sent_at"),                  // stamped when emailed
  firstViewedAt: text("first_viewed_at"),   // first public-share page view
  lastViewedAt: text("last_viewed_at"),     // most recent public-share page view
});
...
export const invoiceSettingsSchema = z.object({
  ...
  design: z.object({
    ...
    // Branding (P3.10): one of three layout templates + an optional logo (a
    // data: URL for a small image, ≤1MB, stored in settings).
    template: z.enum(["classic", "modern", "compact"]).default("classic"),
    logoDataUrl: z.string().max(1_400_000).nullable().default(null),
  }).default({}),
  scheduling: z.object({
...
export const bills = pgTable("bills", {
  ...
  reminderExempt: boolean("reminder_exempt").notNull().default(false), // excluded from reminder batches
});
