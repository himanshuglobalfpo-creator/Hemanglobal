 ...
 * against the REAL storage engine (server/storage.ts + better-sqlite3).
 * This is NOT a mirror test: it exercises the exact code production runs.
 *
 * What it proves:
...
const DB = "/tmp/ll_statement_effect_test.db";
for (const f of [DB, DB + "-wal", DB + "-shm"]) if (fs.existsSync(f)) fs.unlinkSync(f);
process.env.DB_PATH = DB; // must be set BEFORE importing storage (module-level open)
...
// Ledger values are INTEGER CENTS now — equality is EXACT, no epsilon.
// $(dollars) converts a human-readable expectation into cents.
const eq2 = (a: number, b: number) => a === b;
const $ = (dollars: number) => Math.round(dollars * 100);

async function main() {
  // IMPORTANT: dynamic imports — static ESM imports hoist above the env setup
  // and the storage module would open the default ./data.db instead of DB_PATH.
  const { storage } = await import("../server/storage");
  const { withOrg } = await import("../server/org-scope");

  withOrg({ orgId: 1, userId: 0 }, () => {
  // Fresh DB: migration created org 1 and seeded the default chart of accounts.
  ...
  const cust = storage.createCustomer({ name: "Test Customer" });
  ...
  const je = storage.listJournalEntries(10).find((e) => e.reference === "INV-TEST-1")!;
  ...
  const pl = storage.profitAndLoss(DATE, DATE);
  ...
  const bs = storage.balanceSheet(DATE);
  ...
  const tb = storage.trialBalance(DATE);
  ...
  const aging = storage.arAging(DATE);
  ...
  const taxRep = storage.taxLiabilityReport(DATE);
  ...
  const cf1 = storage.cashFlowStatement(DATE, DATE) as any;
  ...
  storage.payInvoice({ invoiceId: inv.id, date: DATE, amount: 110, bankAccountId: BANK.id });

  console.log("9. After payment");
  const bs2 = storage.balanceSheet(DATE);
