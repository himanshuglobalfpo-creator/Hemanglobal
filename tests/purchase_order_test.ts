...
async function main() {
  ...
  if (!process.env.DATABASE_URL) {
    ...
    const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "ledgerlite-po-pg-"));
    const epg = new EmbeddedPostgres({
      databaseDir: dataDir, user: "postgres", password: "password", port: 55447,
      persistent: false, createPostgresUser: (process.getuid?.() ?? 1000) === 0,
    });
    await epg.initialise();
    await epg.start();
    await epg.createDatabase("ledgerlite_po_test");
    process.env.DATABASE_URL = "postgresql://postgres:password@localhost:55447/ledgerlite_po_test";
