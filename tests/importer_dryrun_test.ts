...
// test DB (DATABASE_URL) inside an org context, asserts row counts unchanged.
import { pool, closeDatabase, storage } from "../server/storage";
import { withOrg } from "../server/org-scope";
import { importCustomers, importOpeningBalances } from "../server/importers";
let fail = 0;
...
(async () => {
  ...
  await withOrg({ orgId: 1, userId: 1 }, async () => {
    ...
    const msg = obRep.errors.map((e) => e.message).join(" ");
